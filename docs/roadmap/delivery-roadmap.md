...
**Next implementation slice:** [Forge R2–R4 handoff](../superpowers/plans/2026-09-20-forge-r2-r4-handoff.md). R1 reliability and R2's pure inventory allocation/job-draft partitioning are present. v0.67 expands the durable specialist plan from Bundle 5 to Blueprint-owned Locations and Factions in Bundle 2, preserves resumable v0.66 ledgers, and removes the hidden catch-all lore allocation. The next priority is inventory-driven specialist scheduling for cast, relationships, knowledge, items and secrets, followed by Bundle 1/6 treatment, context preflight, prompt profiles and raw-idea routing. Artifact-target descriptions are reachable in Blueprint Studio. [Forge R1](../superpowers/plans/2026-09-20-forge-r1-executable-blueprint.md) passes automated verification on `test-2-glm`; live provider timing and creative quality remain unmeasured.

**2026-09-20 priority update:** repeated Bundle 5 failures across user-reported Gemini and GLM runs moved Forge stabilization ahead of the next canon-acceptance slice. R1 now uses explicit schema contracts, strict single-document parsing, complete-candidate validation, atomic combined-range commit and one bounded correction for malformed output. The [Forge overhaul and Prompt Studio design](../superpowers/specs/2026-09-20-forge-overhaul-and-prompt-studio.md) and [delivery plan](../superpowers/plans/2026-09-20-forge-overhaul-delivery.md) still cover unshipped R2–R4 work: smaller durable specialist jobs, explicit category allocation, editable versioned creative profiles and the raw-idea route. M4 retrieval/Activation Lab, M5 card production and M6 native fidelity evidence remain separate unfinished work.

**Status:** 11 of 17 ordered roadmap items complete; 6 remain. M2 Blueprint Studio and M3.1 durable Forge integration are complete. v0.58 makes the accepted Blueprint and its independent lore/runtime budgets reachable from Stage 3; v0.59–v0.60 repair repeat access, the Stage 3 editor route, and ordinary default Physics inputs. v0.61 closes the first production-coverage gap: every enabled Blueprint category has a Forge destination, minimum entry/category coverage is enforced before bundle-five acceptance, and new rules/secrets/history require semantic names. v0.62 adds deterministic, read-only cross-link reports for generated relationship and knowledge proposals. v0.55–v0.57 advanced M3.2 from lossless category records to Blueprint-driven Forge and typed specialist projections. Canon acceptance, scoped category regeneration beyond bundle retry, and production retrieval architecture remain in progress. M4.1 foundation work is partially complete. Read the [architecture](production-studio-blueprint.md) before implementing a milestone. Cut a detailed implementation plan per bounded subsystem; do not execute this whole horizon as a single refactor.
...
**Progress:** M3.1 complete. v0.53 added the canonical `lorebible.forge-build/v1` state machine and transactional Project Graph commands. v0.54 connected matching prepared graphs to live Forge and reload recovery. v0.55 introduced stable proposed category records and derived manuscript checkpoints; v0.56 made accepted Blueprint coverage reach live batches. v0.57 adds typed specialist projections for entities, directional relationships, knowledge boundaries, and temporal facts. v0.61 makes that coverage enforceable: legacy and custom categories receive explicit targets and canonical storage, with minimum coverage checked before acceptance. v0.62 resolves unambiguous proposal references into a revision-scoped report without writing canon edges. Explicit canon acceptance and finer scoped regeneration remain before M3.2 exits.
...
Add a first-class **Lore Quality Auditor and Retrieval Linter** before release compilation. It must inspect every enabled entry and the book as a system: semantic title/category accuracy; content focus, specificity and useful depth; entry token profile; primary/secondary key quality; whole-word, regex and case choices; selective logic; scan depth; position/depth/role; order versus priority; probability; sticky/cooldown/delay; groups/weights/override; recursion controls; vector dependency; temporal/secret/knowledge routing; duplicate ownership; contradictions; unresolved references; activation fan-out; dead entries; and typical/crowded budget behavior. Every advanced setting must have a recorded rationale or a finding. It must show entry-level evidence and safe suggested repairs without silently rewriting accepted prose.

Add a mobile-safe **Lorebook Viewer and Editor** over the canonical Lore Manifest. It must support category/search/filter views, entry inspection, content and title/category editing, activation and injection settings, rationale, tests, token estimates, stable IDs, lock state and revision history. Edits use graph commands with validation and blast-radius previews; raw exported JSON is an optional expert view, never a second authority. The editor must compare attached and standalone projections and show stale QA findings after relevant edits.

Content depth is category- and role-aware. Production profiles may propose a roughly 150-token minimum for substantial reference entries, while allowing shorter atomic triggers, routing/index entries, relationship bridges, and intentionally terse rules when their purpose is complete. Flag suspiciously thin entries (including the observed ~25-token failure class) against the accepted build profile and assigned entry purpose; never pad to satisfy a number. Also flag bloated multi-concept entries and recommend splitting them. User-visible controls should expose the selected quality profile, minimum/target range by entry kind, exceptions and waivers.

**Tests:** positive/negative/collision/selective; secret eligibility; seeded groups/probability; persistence message sequences; bounded recursion; crowded-budget eviction; disabled entries; unavailable embedding dependency; keyword fallback limitations; generic/dead/colliding keys; wrong or placeholder titles/categories; unexplained advanced settings; thin, padded and overstuffed entry profiles; redundancy, contradiction and dangling-reference evidence. Cross-check simulator expectations against named native diagnostics where available.

**Exit gate:** no important compiled fact lacks explained ownership/retrieval, no unbounded recursion by default, secrets have explicit protection limitations, unsupported simulation returns unknown. Typical/crowded budgets use disclosed assumptions. Production publishing has no unresolved Auditor Blockers; Majors require repair or an explicit safe waiver, and every finding names the entry, evidence, expected behavior and recommended action.
...
Add a **Project Steward Agent** that the user can chat with about the current project. It may explain, suggest, compare, refine, and prepare scoped change sets across Blueprint, graph canon, cards, lore and retrieval settings. The agent must distinguish discussion from action, show affected records and validation impact before applying changes, preserve locks/history, require confirmation for creative rewrites or broad changes, and commit accepted edits through the same typed command/revision system as the manual UI. It cannot bypass schemas, release gates, evidence labels or native-capability blockers. Conversation summaries and proposed patches are project-scoped; credentials and hidden reasoning are excluded.
