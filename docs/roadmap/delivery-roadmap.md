...
**Next implementation slice:** [Forge R2–R4 handoff](../superpowers/plans/2026-09-20-forge-r2-r4-handoff.md). R1 reliability and the R2 durable specialist path cover all six logical Forge bundles. v0.70 adds visible Stage 3 request preflight. v0.71 adds R3's versioned Forge prompt registry, dedicated persisted application profiles, and a Creative Prompts Settings surface with protected requirements kept read-only. Profiles are not active generation inputs yet. The next priority is project/profile selection plus immutable resolved-prompt snapshots and replay hashes, followed by canon conflict preflight and R4 raw-idea routing. Live provider timing, capacity, and creative quality remain unmeasured.
