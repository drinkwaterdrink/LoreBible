...
**Next implementation slice:** [Forge R2–R4 handoff](../superpowers/plans/2026-09-20-forge-r2-r4-handoff.md). R1 reliability and the R2 durable specialist path cover all six logical Forge bundles. v0.70 adds visible Stage 3 request preflight. v0.71–v0.74 deliver R3's versioned prompt registry, application/project precedence, immutable build snapshots, profile transfer and layer comparison. v0.75 adds the exact compiled-request preview. v0.76 adds an explicit, cancellable disposable model test whose schema-validated candidate cannot mutate project or checkpoint state. The next priority is R3 canon-conflict preflight, followed by R4 raw-idea routing. Live provider timing, capacity, and creative quality remain unmeasured until the user explicitly runs a paid test.
