...
Status: partially implemented. R1 passed. v0.69 completes R2's bounded durable specialist ownership across all six logical bundles, including Bundle 1 structural singletons, Blueprint-owned lore entries, exact cast-tier ownership, cast-before-relationship/knowledge dependencies, retry reuse, bundle-scoped lifecycle handling, split-safe dependency rewiring, and Bundle 6 opening/instruction singletons. Existing v0.66–v0.68 ledgers remain resumable and use the whole-bundle fallback where they lack safe singleton jobs. v0.70 adds deterministic request/context preflight. v0.71 adds the browser-safe versioned Forge prompt registry, dedicated server-persisted application profiles, optimistic revisions, protected-requirement disclosure, and the first Creative Prompts Settings surface. Profiles remain authoring drafts and are not yet applied to generation; project selection, immutable resolved snapshots/replay hashes, canon conflict preflight, and R4 raw-idea routing remain planned. This continuation makes downstream choices explicit; it is not authorization to combine all slices into one unreviewed change.
...
- [x] Job schema owns exactly its destinations; supplemental job accepts one category only. (Implemented across all six logical bundles.)
...
- [x] Retry/split ceilings; partitioned child IDs cover exactly parent slots; no duplicated content/events. (Implemented for all enabled specialists; singleton jobs retry without fake entry splitting.)
...
6. Preview effective prompt (explicit private-canon disclosure); test on a disposable candidate, never the accepted manuscript. **v0.75–v0.76:** compiled preview and cancellable schema-validated disposable testing are shipped; neither path can mutate the manuscript or Forge checkpoint.
...
- [x] Save costs zero calls; compiled preview is explicit and generation-free; isolated tests expose cost, validate the real schema, support cancellation, exclude reasoning, and have no manuscript/checkpoint mutation dependency.
