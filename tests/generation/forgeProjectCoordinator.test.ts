...
import {createForgeInputFingerprint,createForgeProjectCoordinator} from "../../server/generation/forgeProjectCoordinator";
...
const promptSnapshot=resolvePromptSnapshot({profile:null});
...
test("coordinates a real persisted bundle and resumes it through a fresh repository",async()=>{
  ...
  const prepared=await coordinator.prepare("p",source,"step_by_step",promptSnapshot);expect(prepared.resumeSections).toEqual({});
  ...
  const resumed=await createForgeProjectCoordinator(createProjectRepository(dir),{now:()=>"later",id:(kind)=>`${kind}/two`}).prepare("p",source,"step_by_step",promptSnapshot);
  expect(resumed.nextBundleIndex).toBe(1);expect(resumed.resumeSections).toEqual({core:{},user:{},worldPhysics:{},status:{}});
});

test("a paused Forge build resumes its immutable prompt snapshot after the selected profile changes",async()=>{
  const dir=await mkdtemp(join(tmpdir(),"lb-forge-prompts-"));const repo=createProjectRepository(dir);await repo.create(graph());let sequence=0;
  const coordinator=createForgeProjectCoordinator(repo,{now:()=>"x",id:(kind)=>`${kind}/${++sequence}`});
  const original=resolvePromptSnapshot({profile:{schemaVersion:1,id:"profile/a",name:"A",revision:1,overrides:[{featureId:"forge.core",baseVersion:1,text:"Original direction",revision:1}]}});
  const changed=resolvePromptSnapshot({profile:{schemaVersion:1,id:"profile/a",name:"A",revision:2,overrides:[{featureId:"forge.core",baseVersion:1,text:"Changed direction",revision:2}]}});
  const first=await coordinator.prepare("p",source,"continuous",original);
  const resumed=await coordinator.prepare("p",source,"continuous",changed);
  expect(resumed.buildId).toBe(first.buildId);
  expect(resumed.promptSnapshot.hash).toBe(original.hash);
  expect(resumed.promptSnapshot.resolvedCreativeText["forge.core"]).toBe("Original direction");
});

test("a legacy checkpoint snapshots only its remaining work and records prior prompt provenance",async()=>{
  const dir=await mkdtemp(join(tmpdir(),"lb-forge-legacy-prompts-"));const repo=createProjectRepository(dir);const initial=graph();const sourceForLegacy={...source};let build=createForgeBuild({id:"build/legacy",sourceRevision:1,inputFingerprint:createForgeInputFingerprint(sourceForLegacy),executionMode:"continuous",createdAt:"old"});build=beginForgeBatch(build,{bundleIndex:0,attemptId:"old/one",provider:"gemini",modelId:"flash",route:"gemini_native",startedAt:"old"});build=completeForgeBatch(build,{bundleIndex:0,attemptId:"old/one",commandId:"accepted/one",sections:{core:{},user:{},worldPhysics:{},status:{}},completedAt:"old"});initial.project.revision=2;initial.builds=[build];await repo.create(initial);
  const coordinator=createForgeProjectCoordinator(repo,{now:()=>"new",id:(kind)=>`${kind}/new`});
  const resumed=await coordinator.prepare("p",sourceForLegacy,"continuous",promptSnapshot);const saved=resumed.graph.builds[0]as ForgeBuildRecordV1;
  expect(saved.checkpoint.completedBundleCount).toBe(1);expect(saved.promptSnapshot?.hash).toBe(promptSnapshot.hash);expect(saved.legacyPromptCompletedBundleCount).toBe(1);
});

test("an interrupted active attempt is made retryable without losing its checkpoint",async()=>{
  const dir=await mkdtemp(join(tmpdir(),"lb-forge-coordinator-"));const repo=createProjectRepository(dir);await repo.create(graph());
  const coordinator=createForgeProjectCoordinator(repo,{now:()=>"x",id:(kind)=>`${kind}/one`});const prepared=await coordinator.prepare("p",source,"continuous",promptSnapshot);
  await coordinator.begin(prepared,{bundleIndex:0,provider:"nanogpt",modelId:"google/gemini-flash",route:"openai_compatible"});
  const recovered=await createForgeProjectCoordinator(createProjectRepository(dir),{now:()=>"y",id:(kind)=>`${kind}/two`}).prepare("p",source,"continuous",promptSnapshot);
...
test("fingerprints creative inputs but permits model changes",async()=>{
  ...
  const first=await coordinator.prepare("p",source,"continuous",promptSnapshot);const same=await coordinator.prepare("p",structuredClone(source),"continuous",promptSnapshot);expect(same.buildId).toBe(first.buildId);
  const changed=await coordinator.prepare("p",{...source,sparkText:"A mountain"},"continuous",promptSnapshot);expect(changed.buildId).not.toBe(first.buildId);
});

test("accepts one provider response as six atomic bundle checkpoints",async()=>{
  const dir=await mkdtemp(join(tmpdir(),"lb-forge-coordinator-"));const repo=createProjectRepository(dir);await repo.create(graph());let sequence=0;const coordinator=createForgeProjectCoordinator(repo,{now:()=>"x",id:(kind)=>`${kind}/${++sequence}`});
  const prepared=await coordinator.prepare("p",source,"single_request",promptSnapshot);const provenance={provider:"gemini",modelId:"gemini-2.5-flash",route:"openai_compatible"};const active=await coordinator.begin(prepared,{bundleIndex:0,...provenance});
...
test("a new coordinator resumes saved Bundle 5 jobs without repeating completed work",async()=>{
  ...
  const initial=graph();
  let build=createForgeBuild({id:"build/existing",sourceRevision:1,inputFingerprint:"sha256:source",executionMode:"continuous",createdAt:"now",promptSnapshot});
  ...
  const prepared={projectId:"p",buildId:build.id,sourceRevision:1,inputFingerprint:build.inputFingerprint,promptSnapshot,graph:initial,resumeSections:build.checkpoint.sections,nextBundleIndex:4,complete:false};
