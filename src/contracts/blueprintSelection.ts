...
export function parseBlueprintSelectionV1(input:unknown):BlueprintSelectionParseResult{
  try{
    ...
    if(Array.isArray(value.artifactTargets)&&value.artifactTargets.length===0)issues.push({path:"artifactTargets",message:"Choose at least one artifact target."});
    ...
    if(Array.isArray(value.lockedFields)&&new Set(value.lockedFields).size!==value.lockedFields.length)issues.push({path:"lockedFields",message:"Locked fields must be unique."});
