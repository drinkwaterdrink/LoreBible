...
export type BlueprintLorebookScale = "compact" | "standard" | "large" | "massive";
...
export interface MechanicPackRecommendation { id: string; label: string; status: MechanicPackStatus; reason: string; evidenceRefs: string[]; architectureEffects: string[]; runtimeRequirements: string[]; compilerRules: string[]; testFixtures: string[]; gracefulFallback: string; }
...
export interface BlueprintPlanV1 {
  ...
  lorebookScale: BlueprintRecommendation<BlueprintLorebookScale>;
...
const lorebookScales = ["compact", "standard", "large", "massive"] as const;
...
function parseBlueprintPlanInternal(value: unknown): BlueprintParseResult<BlueprintPlanV1> {
  const issues: BlueprintParseIssue[] = [];
  inspectSafety(value, "", issues);
  if (!closed(value, "", ["schema", "source", "interfaceMode", "artifactTargets", "worldMode", "buildIntensity", "generationQuality", "runtimeBudget", "lorebookScale", "lorebookRange", "principalCastRange", "rosterCastRange", "categories", "mechanicPacks", "assessments", "inventory", "findings", "createdAt"], issues)) return { ok: false, issues };
  ...
  recommendation(value.lorebookScale, "lorebookScale", issues, lorebookScales);
  range(value.lorebookRange, "lorebookRange", issues); range(value.principalCastRange, "principalCastRange", issues); range(value.rosterCastRange, "rosterCastRange", issues);
  ...
  if (!Array.isArray(value.mechanicPacks)) issue(issues, "mechanicPacks", "Expected mechanic packs."); else { uniqueIds(value.mechanicPacks, "mechanicPacks", issues); value.mechanicPacks.forEach((item, index) => { const path = `mechanicPacks[${index}]`; if (!closed(item, path, ["id", "label", "status", "reason", "evidenceRefs", "architectureEffects", "runtimeRequirements", "compilerRules", "testFixtures", "gracefulFallback"], issues)) return; string(item.label, `${path}.label`, issues); enumValue(item.status, mechanicStatuses, `${path}.status`, issues); string(item.reason, `${path}.reason`, issues); stringArray(item.evidenceRefs, `${path}.evidenceRefs`, issues, true); for(const key of ["architectureEffects","runtimeRequirements","compilerRules","testFixtures"] as const) stringArray(item[key],`${path}.${key}`,issues,key!=="runtimeRequirements"); string(item.gracefulFallback,`${path}.gracefulFallback`,issues); }); }
