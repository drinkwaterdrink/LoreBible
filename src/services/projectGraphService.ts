...
export async function loadForgeRecovery(id:string,source:ForgeRecoverySource){return json<{graph:ProjectGraphV1;recovery:ForgeRecovery|null}>(await fetch(`/api/projects/graph/${encodeURIComponent(id)}/forge-recovery`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(source)}));}
export async function renameGraphEntity(id:string,revision:number,entityId:string,name:string){return json<ProjectGraphChangeResult>(await fetch(`/api/projects/graph/${encodeURIComponent(id)}/commands`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({commandId:createClientId("command"),expectedRevision:revision,issuedAt:new Date().toISOString(),command:{type:"entity.rename",entityId,name}})}));}
...
export async function previewBlueprint(projectId:string,request:BlueprintPreviewRequestV1,signal?:AbortSignal):Promise<BlueprintPlanV1>{const body=await json<{plan:unknown}>(await fetch(`/api/projects/graph/${encodeURIComponent(projectId)}/blueprint-preview`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({expectedRevision:request.expectedRevision,context:request.context}),signal}));const parsed=parseBlueprintPlan(body.plan);if(!parsed.ok)throw new ProjectGraphApiError("Blueprint response was invalid.","invalid_blueprint_response",502);return parsed.value;}
