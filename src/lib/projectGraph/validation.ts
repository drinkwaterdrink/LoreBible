...
export function parseProjectGraph(value: unknown): ProjectGraphParseResult {
  ...
  (value.builds as any[]).forEach((item, index) => {
    ...
    if(item.promptSnapshot!==undefined){const snapshot=item.promptSnapshot;const valid=record(snapshot)&&snapshot.schemaVersion===1&&typeof snapshot.registryVersion==="string"&&typeof snapshot.hash==="string"&&/^sha256:[a-f0-9]{64}$/.test(snapshot.hash)&&record(snapshot.resolvedCreativeText)&&Object.values(snapshot.resolvedCreativeText).every(value=>typeof value==="string")&&(snapshot.profileId===null||typeof snapshot.profileId==="string")&&(snapshot.profileRevision===null||Number.isSafeInteger(snapshot.profileRevision));if(!valid)issues.push({path:`${prefix}.promptSnapshot`,message:"Forge prompt snapshot is invalid."});}
    if(item.legacyPromptCompletedBundleCount!==undefined&&(!Number.isSafeInteger(item.legacyPromptCompletedBundleCount)||item.legacyPromptCompletedBundleCount<0||item.legacyPromptCompletedBundleCount>6))issues.push({path:`${prefix}.legacyPromptCompletedBundleCount`,message:"Legacy prompt provenance count is invalid."});
    ...
    if(item.specialistLedger!==undefined){
      ...
      if(!record(ledger)||ledger.version!==1||typeof ledger.planHash!=="string"||!ledger.planHash||ledger.inputFingerprint!==item.inputFingerprint||!Array.isArray(ledger.jobs))issues.push({path,message:"Forge specialist ledger identity is invalid."});
        ...
        for(const batch of item.batches??[])if(batch?.status==="complete"&&ledger.jobs.some((candidate:any)=>candidate.job.bundleIndex===batch.index)&&canonicalizeJson(mergeSpecialistJobs(ledger as ForgeSpecialistLedgerV1,batch.index))!==canonicalizeJson(batch.sections))throw Error(`Completed Bundle ${batch.index+1} differs from saved specialist jobs.`);
