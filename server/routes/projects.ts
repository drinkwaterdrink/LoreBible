...
import { findRecoverableForgeBuild } from "../../src/lib/projectGraph/forgeBuilds.js";
import { createForgeInputFingerprint, type ForgeCreativeSource } from "../generation/forgeProjectCoordinator.js";
...
export function registerProjectRoutes(app:Express,{repository}:{repository:ProjectRepository}){
  ...
  app.post("/api/projects/graph/:projectId/blueprint-preview",async(req,res)=>{try{const parsed=parseBlueprintPreviewRequest(req.body);if("issues" in parsed){const rejected=blueprintRequestError(parsed.issues);return res.status(rejected.status).json({error:rejected.error});}const graph=await repository.load(req.params.projectId);if(!graph)return res.status(404).json({error:{code:"missing",message:"Project was not found."}});if(graph.project.revision!==parsed.value.expectedRevision)return res.status(409).json({error:blueprintRevisionError(parsed.value.expectedRevision,graph.project.revision!)});const submitted=parsed.value.context;const context={projectId:graph.project.id,projectRevision:graph.project.revision!,...(submitted.sparkDna===undefined?{}:{sparkDna:submitted.sparkDna}),...(submitted.selectedTake===undefined?{}:{selectedTake:submitted.selectedTake}),...(submitted.physicsConstraints===undefined?{}:{physicsConstraints:submitted.physicsConstraints}),...(submitted.generationQuality===undefined?{}:{generationQuality:submitted.generationQuality})};return res.json({plan:createBlueprintPlan({graph,context},{createdAt:new Date().toISOString()})});}catch(e){return failure(res,e);}});
  app.post("/api/projects/graph/:projectId/forge-recovery",async(req,res)=>{try{const graph=await repository.load(req.params.projectId);if(!graph)return res.status(404).json({error:{code:"missing",message:"Project was not found."}});const body=req.body&&typeof req.body==="object"?req.body:{};const source:ForgeCreativeSource={sparkText:body.sparkText,parse:body.parse,canon:body.canon,physics:body.physics,chosenTake:body.chosenTake,blueprintSelection:body.blueprintSelection};const recovery=findRecoverableForgeBuild(graph,createForgeInputFingerprint(source));return res.json({graph,recovery});}catch(e){return failure(res,e);}});
