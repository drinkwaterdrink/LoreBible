...
export function registerProjectRoutes(app:Express,{repository}:{repository:ProjectRepository}){
  ...
  app.post("/api/projects/graph/:projectId/forge-recovery",async(req,res)=>{try{const graph=await repository.load(req.params.projectId);if(!graph)return res.status(404).json({error:{code:"missing",message:"Project was not found."}});const body=req.body&&typeof req.body==="object"?req.body:{};const source:ForgeCreativeSource={sparkText:body.sparkText,parse:body.parse,canon:body.canon,physics:body.physics,chosenTake:body.chosenTake};const recovery=findRecoverableForgeBuild(graph,createForgeInputFingerprint(source));return res.json({graph,recovery});}catch(e){return failure(res,e);}});
