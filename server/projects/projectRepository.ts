...
export interface ProjectRepositorySummary{id:string;name:string|null;revision:number;status:string;legacyDocumentId:string|null}
...
export function createProjectRepository(root:string):ProjectRepository{
  ...
  const repo:ProjectRepository={
    async list(){try{const dirs=await readdir(root,{withFileTypes:true});const out:ProjectRepositorySummary[]=[];for(const d of dirs)if(d.isDirectory())try{const g=await parsedFile(join(root,d.name,"project.json"),"corrupt_primary");if(g)out.push({id:g.project.id,name:g.project.name,revision:g.project.revision!,status:g.project.status,legacyDocumentId:typeof g.extensions.legacyDocumentId==="string"?g.extensions.legacyDocumentId:null});}catch{}return out;}catch(e){if(missing(e))return[];throw new ProjectRepositoryError("Repository could not be listed.","io");}},
